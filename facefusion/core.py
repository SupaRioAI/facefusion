...
from facefusion.execution_helper import (
...
def cli() -> None:
    ...
    group_execution.add_argument(
        "--execution-providers",
        help=wording.get("execution_providers_help").format(
    ...
    group_execution.add_argument(
        "--max-memory",
        help=wording.get("max_memory_help"),
        type=int,
        choices=facefusion.choices.max_memory_range,
        metavar=create_metavar(facefusion.choices.max_memory_range),
    )
    # face analyser
    ...
    group_face_selector.add_argument(
        "--face-selector-mode",
        help=wording.get("face_selector_mode_help"),
        default="many",
        choices=facefusion.choices.face_selector_modes,
    )
    group_face_selector.add_argument(
        "--reference-face-position",
        help=wording.get("reference_face_position_help"),
        type=int,
        default=0,
    )
    group_face_selector.add_argument(
        "--reference-face-distance",
        help=wording.get("reference_face_distance_help"),
        type=float,
        default=0.6,
    ...
    group_face_selector.add_argument(
        ...
        help=wording.get("reference_frame_number_help"),
        type=int,
        default=0,
    )
    # face mask
    ...
    group_face_mask.add_argument(
        "--face-mask-types",
        help=wording.get("face_mask_types_help").format(
    ...
    group_face_mask.add_argument(
        "--face-mask-blur",
        help=wording.get("face_mask_blur_help"),
        type=float,
        default=0.3,
    ...
    group_face_mask.add_argument(
        "--face-mask-padding",
        help=wording.get("face_mask_padding_help"),
        type=int,
        default=[30, 10, 38, 10],
        nargs="+",
    )
    group_face_mask.add_argument(
        "--face-mask-regions",
        help=wording.get("face_mask_regions_help").format(
            choices=", ".join(facefusion.choices.face_mask_regions)
        ),
        default=facefusion.choices.face_mask_regions,
    ...
    group_output_creation.add_argument(
        ...
        help=wording.get("output_image_quality_help"),
        type=int,
        default=80,
    ...
    group_output_creation.add_argument(
        ...
        help=wording.get("output_video_encoder_help"),
        default="libx265",
        choices=facefusion.choices.output_video_encoders,
    )
    group_output_creation.add_argument(
        "--output-video-quality",
        help=wording.get("output_video_quality_help"),
        type=int,
        default=80,
...
def process_video(start_time: float) -> None:
    ...
    # extract frames
    logger.info(
        wording.get("extracting_frames_fps").format(
            video_fps=facefusion.globals.output_video_fps
        ),
        __name__.upper(),
    )
    extract_frames(
    ...
    # merge video
    logger.info(
        wording.get("merging_video_fps").format(
            video_fps=facefusion.globals.output_video_fps
        ),
        __name__.upper(),
    )
