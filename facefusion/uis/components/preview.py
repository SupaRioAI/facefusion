...
PREVIEW_SKIP_SLIDER: Optional[gradio.Slider] = None


def render() -> None:
    global PREVIEW_IMAGE
    global PREVIEW_FRAME_SLIDER
    global PREVIEW_SKIP_SLIDER
    ...
    preview_skip_slider_args: Dict[str, Any] = {
        "value": 10,
        "maximum": 1000,
        "label": "SKIP",
        "info": "Click on Preview to skip frames",
        "visible": False,
    }
    ...
    if is_video(facefusion.globals.target_path):
        ...
        preview_skip_slider_args["visible"] = True
    PREVIEW_IMAGE = gradio.Image(**preview_image_args)
    PREVIEW_FRAME_SLIDER = gradio.Slider(**preview_frame_slider_args)
    PREVIEW_SKIP_SLIDER = gradio.Slider(**preview_skip_slider_args)
    register_ui_component("preview_frame_slider", PREVIEW_FRAME_SLIDER)
    register_ui_component("preview_skip_slider", PREVIEW_SKIP_SLIDER)


def listen() -> None:
    PREVIEW_IMAGE.select(
        preview_skip, inputs=PREVIEW_FRAME_SLIDER, outputs=PREVIEW_FRAME_SLIDER
    )
    ...
    multi_two_component_names: List[ComponentName] = ["target_image", "target_video"]
    ...
    for frame_processor in facefusion.globals.frame_processors:
        ...
        if frame_processor_module.pre_process("preview"):
            temp_frame = frame_processor_module.process_frame(
                source_face, reference_faces, temp_frame
            )
    return temp_frame


def preview_skip(frame_slider) -> gradio.Slider:
    if is_video(facefusion.globals.target_path):
        video_frame_total = count_video_frame_total(facefusion.globals.target_path)
        skip_2_frame = (frame_slider + PREVIEW_SKIP_SLIDER.value) % video_frame_total
        return gradio.Slider(
            value=skip_2_frame, maximum=video_frame_total, visible=True
        )
    return gradio.Slider(value=None, maximum=None, visible=False)
