...
def render() -> None:
    ...
    WEBCAM_IMAGE = gradio.Image(label=wording.get("webcam_image_label"))
    WEBCAM_START_BUTTON = gradio.Button(
        value=wording.get("start_button_label"), variant="primary", size="sm"
    )
    WEBCAM_STOP_BUTTON = gradio.Button(
        value=wording.get("stop_button_label"), size="sm"
    )
...
def listen() -> None:
    ...
    WEBCAM_STOP_BUTTON.click(stop, cancels=start_event, concurrency_limit=10)
